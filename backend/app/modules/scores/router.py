...
from app.modules.revisions.schemas import FingeringRevisionRequest, RevisionCreateRequest
...
from app.modules.scores.schemas import ScoreBatchDeleteRequest, ScoreUpdateRequest
...
async def list_scores(
    ...
    return paginated_response(
        [item.model_dump() for item in items], page, page_size, total
    )
...
async def get_score(
    ...
    user_id = require_persisted_id(current_user.id, entity="user")
    return success_response(data=(await service.get(db, score_id, user_id)).model_dump())
...
async def approve_score(
    ...
    return success_response(data=result.model_dump(), message=SuccessCode.RECOGNITION_CONFIRMED)


@router.get("/{score_id}/revisions")
...
@router.post("/{score_id}/revisions")
async def create_revision(
    ...
    return success_response(data=result.model_dump(), message=SuccessCode.SAVE_SUCCESS)


@router.get("/{score_id}/revisions/{revision_id}/content")
...
@router.post("/{score_id}/fingering")
async def generate_score_fingering(
    ...
    return success_response(data=result.model_dump(), message=SuccessCode.FINGERING_GENERATED)
