...
from app.processing.engines.practice_alignment.matchmaker_live import (  # noqa: E402
...
async def resolve_revision_xml(score_uuid: str, revision_uuid: str | None) -> str:
    async with AsyncSessionLocal() as db:
        ...
        if not source:
            raise RuntimeError(
                f"Canonical MusicXML not found for revision: {revision.revision_uuid}"
            )
...
async def main() -> int:
    ...
    try:
        path = (
            ...
            if args.xml_path
            else await resolve_revision_xml(args.score_id, args.revision_id)
        )
