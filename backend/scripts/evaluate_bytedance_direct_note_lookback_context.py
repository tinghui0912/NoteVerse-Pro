"""Measure past lookback needed by ByteDance direct note_model.

This research-only script uses development/calibration cases only. It keeps the
frozen verifier fixed, uses +220 ms future context, and compares shorter
lookback windows against a 1000 ms lookback reference.
"""
...
LOOKBACK_MS_VALUES = (150, 250, 500, 750, 1000)
REFERENCE_LOOKBACK_MS = 1000
...
def main() -> int:
    ...
    for device in args.device:
        ...
        lookback_results = {
            ...
            for lookback_ms in LOOKBACK_MS_VALUES
        }
        reference = lookback_results[REFERENCE_LOOKBACK_MS]
        device_reports[device] = {
            "lookbacks": {
                str(lookback_ms): _lookback_summary(
                    lookback_results[lookback_ms],
                    reference=reference,
                    lookback_ms=lookback_ms,
                )
                for lookback_ms in LOOKBACK_MS_VALUES
            ...
            "cases": {
                str(lookback_ms): lookback_results[lookback_ms]
                for lookback_ms in LOOKBACK_MS_VALUES
            },
        }

    report = {
        "benchmark_scope": "bytedance_direct_note_lookback_context",
        ...
        "lookback_ms_values": list(LOOKBACK_MS_VALUES),
        "reference_lookback_ms": REFERENCE_LOOKBACK_MS,
        "frontend_semantics": {
            "model": "ByteDance note_model only",
            "window": "[target - lookback, target + 220ms]",
        ...
        "stft_context_note": {
            ...
            "minimum_left_support_ms": 114,
            "reason": "local evidence starts at target -50ms and centered STFT needs roughly another 64ms of real past audio",
        },
...
def _evaluate_case(
    ...
    *,
    ...
    for index, expected_pitches in enumerate(expected_groups):
        ...
        clip_start_seconds = max(0.0, relative_target - lookback_ms / 1000.0)
        clip_end_seconds = min(audio.size / sample_rate, relative_target + FUTURE_PREFIX_MS / 1000.0)
        clip_audio = audio[
            int(round(clip_start_seconds * sample_rate)) : int(round(clip_end_seconds * sample_rate))
        ]
        clip_path = (
            work_dir
            / f"bytedance_direct_note_lookback{lookback_ms}_{device}"
            / (
                f"{case['case_id']}_g{index + 1:02d}"
                f"_lookback{lookback_ms}_future{FUTURE_PREFIX_MS}.wav"
            )
        )
        ...
        prediction = _activation_prediction(
            ...
            clip_start_seconds=source_start + clip_start_seconds,
        ...
        group_results.append(
            {
                ...
                "clip_window": {
                    "start_relative_to_target_ms": round((clip_start_seconds - relative_target) * 1000.0),
                    "end_relative_to_target_ms": round((clip_end_seconds - relative_target) * 1000.0),
                },
            }
        )
...
def _disagreement_row(
            ...
            rows.append(
                {
                    ...
                    "short_lookback_onset": evidence.get("onset_activation"),
                    "short_lookback_frame": evidence.get("frame_activation"),
    ...
    return {
        ...
        "short_lookback_decision": bool(evaluation["accepted"]),
        "reference_1000ms_decision": bool(reference["accepted"]),
