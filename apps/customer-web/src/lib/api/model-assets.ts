...
export function assertModelAssetAccess(data: unknown): asserts data is ModelAssetAccess {
  ...
  if (!candidate.assetId || typeof candidate.assetId !== 'string') {
    throw new Error('ModelAssetAccess missing assetId');
  }
  if (!candidate.assetVersion || typeof candidate.assetVersion !== 'string') {
    throw new Error('ModelAssetAccess missing assetVersion');
  }
