...
- The score view keeps one persistent bottom control dock with reserved score padding, so pause and end controls remain accessible while scrolling.
- Session status is a compact top status bar; practice commands live in the bottom dock.
...
- Practice settings use a right-side drawer: it starts closed, closes when practice begins, and can be opened from the bottom control dock.
