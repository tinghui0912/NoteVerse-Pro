...
const scoreId = 'score-results-layout';
const revisionId = 'revision-results-layout';
...
test('results uses score/revision resources and keeps playback controls available', async ({ context, page }) => {
  await context.addCookies([{ name: 'noteverse_session', value: 'session', domain: 'localhost', path: '/' }]);
  const score = { score_id: scoreId, title: 'Layout Score', difficulty: 'difficultyIntermediate', state: 'ACTIVE', version: 1,
  ...
  await page.route(`**/api/v1/scores/${scoreId}/artifacts**`, (route) => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ success: true, data: [] }) }));
  ...
  await page.goto(`/en/results/${scoreId}`);
  ...
  await expect(page.getByTestId('results-actions').getByRole('button', { name: 'Create Share' })).toBeVisible();
});
